//Доработал https://www.npmjs.com/package/font-face-generator под свои нужды
...
export default (options = {}) => {
    ...
    if (fs.existsSync(fontFaceFile)) {
        try {
            fs.unlink(fontFaceFile, err => {
                if (err) throw err;
                console.log(
                    `File ${fontFaceFile} removed`
                );
            });
        } catch (e) {
            console.log(
    ...
    const transformStream = new Transform({
        ...
        transform(file, enc, callback) {
            ...
            if (uniqueFonts.has(fileName)) {
                ...
                //let new_fontFamily = fontFamily.split(' ')[0];
                let new_fontFamily = fontFamily;
